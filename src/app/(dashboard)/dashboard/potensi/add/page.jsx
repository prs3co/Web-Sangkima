...
export default function AddPotensi() {
  ...
  async function handleOnSubmit(event) {
    ...
    try {
      if (!accessToken || !verifyJwt(accessToken)) {
        ...
        const dataAdd = await fetch('/api/potency', {
          method: 'POST',
          headers: {
          ...
          <form className="" onSubmit={handleOnSubmit} onChange={handleOnChange} >
            <div>
              <div className="mb-2 block">
                <Label className="text-gray-700" value="Name : " />
              </div>
              <TextInput type="text" name='title' required />
            </div>
            <div>
              <div className='my-3 block'>
                <Image className='h-auto max-w-xl rounded-lg shadow-xl dark:shadow-gray-800' src={imageSrc} alt='image' width={300} height={400} />
              </div>
              <div className="mb-2 block">
